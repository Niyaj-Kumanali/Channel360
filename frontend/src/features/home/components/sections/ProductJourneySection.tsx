...
export const ProductJourneySection: React.FC<Props> = ({ section }) => {
  ...
  return (
    <section ref={sectionRef} className="flex py-16 min-h-[calc(100vh-4rem)] flex-col items-center justify-center overflow-hidden">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 relative">
          ...
          {/* Mobile arrows — down from icon bottom, up from icon top */}
            ...
            {iconPositions.length > 0 && (
              <>
                {Array.from({ length: steps.length }, (_, i) => (
                  <div key={`d${i}`} className="absolute left-0 flex justify-center w-8" style={{ top: (iconPositions[i] ?? 0) + 24 }}>
                    <svg width="12" height="16" viewBox="0 0 12 16" fill="none" className="text-primary/60">
                      ...
                      <polyline points="2,6 6,12 10,6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </div>
                ))}
                {Array.from({ length: steps.length }, (_, i) => (
