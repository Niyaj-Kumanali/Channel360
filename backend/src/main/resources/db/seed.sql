...
-- Fix Footer title to match frontend fallback
UPDATE homepage_sections SET title = 'Footer' WHERE section_name = 'Footer' AND title IS NULL;

-- Homepage popups (seed data for CMS-managed popups)
INSERT INTO homepage_popups (title, description, cta_button_text, cta_url, priority, active, created_by, created_at, updated_at)
SELECT v.* FROM (VALUES
  ('Welcome to Channel360', 'Discover end-to-end visibility across your entire channel ecosystem. Track products from manufacturer to end-customer activation.', 'Explore Platform', '/login', 1, TRUE, 'system', NOW(), NOW()),
  ('New Partner Program Launching Q3', 'Enhanced incentive tiers and streamlined onboarding for all channel partners. Join us for the launch event.', 'Learn More', '#', 2, FALSE, 'system', NOW(), NOW())
) AS v(title, description, cta_button_text, cta_url, priority, active, created_by, created_at, updated_at)
WHERE NOT EXISTS (SELECT 1 FROM homepage_popups);
