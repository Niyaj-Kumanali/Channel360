CREATE OR REPLACE PROCEDURE sp_homepage_sections_save(
    INOUT p_id BIGINT,
...
BEGIN
    IF p_id IS NULL THEN
        ...
        VALUES (p_section_name, p_section_type, p_title, p_subtitle, p_description, p_image_url, p_button_text, p_button_url, p_display_order, COALESCE(p_active, FALSE), p_start_date, p_end_date, p_created_by, NOW(), NOW())
...
CREATE OR REPLACE PROCEDURE sp_homepage_sections_list(
    INOUT p_data REFCURSOR
)
LANGUAGE plpgsql AS $$
BEGIN
...
DECLARE
    item JSON;
BEGIN
    FOR item IN SELECT * FROM json_array_elements(p_json::json)
    LOOP
        UPDATE homepage_sections
        SET display_order = (item->>'displayOrder')::INTEGER, updated_at = NOW()
