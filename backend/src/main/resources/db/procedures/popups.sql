CREATE OR REPLACE PROCEDURE sp_homepage_popups_create(
    IN p_title VARCHAR,
...
BEGIN
    ...
    VALUES (p_title, p_description, p_image_url, p_cta_button_text, p_cta_url, p_priority, p_active, p_start_date, p_end_date, p_created_by, NOW(), NOW())
    RETURNING id INTO p_id;
END;
$$;

CREATE OR REPLACE PROCEDURE sp_homepage_popups_update(
    IN p_id BIGINT,
    IN p_title VARCHAR,
    IN p_description TEXT,
    IN p_image_url VARCHAR,
...
CREATE OR REPLACE PROCEDURE sp_homepage_popups_get_by_id(
    IN p_id BIGINT,
...
CREATE OR REPLACE PROCEDURE sp_homepage_popups_get_all(
    INOUT p_data REFCURSOR
)
LANGUAGE plpgsql AS $$
BEGIN
    OPEN p_data FOR SELECT * FROM homepage_popups WHERE deleted_flag = FALSE ORDER BY priority DESC;
END;
$$;
...
CREATE OR REPLACE PROCEDURE sp_homepage_popups_exists(
    IN p_id BIGINT,
    OUT p_exists BOOLEAN
)
LANGUAGE plpgsql AS $$
BEGIN
    SELECT EXISTS(SELECT 1 FROM homepage_popups WHERE id = p_id AND deleted_flag = FALSE) INTO p_exists;
END;
$$;
